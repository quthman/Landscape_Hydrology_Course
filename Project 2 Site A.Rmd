...
inter_arr_time$dry[which(inter_arr_time$dry==0)] <- NA
...
#RAINSIMULATION
#library(Ecohydmod)
#rainsimulation <- RainPoisson(365,lambda = lamda_p, alpha = #alpha_p_mean/10)
#sum(rainsimulation)
...
dt_daily_rainfall <- cbind(daily_365_rainfall, rainsimulation, alpha_t)
...
vals$Observed_P[which(vals$Observed_P==0)] <- NA
vals$rainsimulation[which(vals$rainsimulation==0)] <- NA

ecdfplot(~ Observed_P + rainsimulation + Theoretical_P, 
         data=vals, auto.key=list(space='right'),
         xlab = "Precipitation", ylab = "Quantile")
